...
export const demoCampaign: Campaign = {
  strategy: {
    ...
    // 30-day split: must sum to budget30(demoIntake) = 400.
    budgetSplit: [
      { item: "Instagram boosted posts (4 × €35)", eur: 140 },
      { item: "Coffee card programme", eur: 90 },
      { item: "Photo session", eur: 60 },
      { item: "Morning tasting", eur: 50 },
    ...
    kpis: [
      {
        ...
        target: "41/day baseline → 51/day by week 4 (+25%)",
        where:
          "POS daily Z-report, filtered Mon–Fri to that window, logged every Monday",
      },
      {
        name: "Google Business Profile direction requests",
        target: "45/month → 80/month by week 4",
        where: "Business Profile performance tab, first Monday of the month",
      },
      {
  ...
  calendar: [
    ...
    },
    {
      ...
      channel: "Google Business Profile",
      assetType: "Post",
      title: "Weekly post: the first hour, hours and prices on the profile",
      minutes: 45,
    },
  ],
  copy: [
