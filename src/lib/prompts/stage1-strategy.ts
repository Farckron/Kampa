...
/** The intake asks for a MONTHLY budget (SPEC §4.2 q4) and the plan covers 30
 *  days, so the monthly figure is the whole pot. This is the pot the model is
 *  told about and the pot checkBudget() validates the split against — they must
 *  never disagree. */
export function budget30(intake: Intake): number {
  return intake.budget ?? 0;
}
...
export function intakeSections(intake: Intake): string {
  return [
    ...
    `BUDGET €: ${intake.budget === null ? "(not given)" : `${budget30(intake)} EUR total for the 30 days`}`,
    ...
    `30-DAY GOAL: ${intake.goal || "(not given)"}`,
...
- icp: the one buyer worth chasing for the next 30 days. Be specific enough that the owner can picture a real person: age range, situation, what triggers the purchase, where they already are.
