...
export function buildCalendarPrompt(
  ...
  const task = `TASK
Produce the 4-week calendar JSON.

- Use ONLY these channels: ${channels}. Nothing else, not even once.
- Cover all 4 weeks. At least one item per week, at least 4 items total.
...
- Front-load setup in week 1 (profile fixes, list, templates), then a steady rhythm. No empty weeks.
