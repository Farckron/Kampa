...
- [x] `anthropic.ts`: fetch wrapper — headers per SPEC §2.4, SSE parser
      ...
      fixtures
- [x] `cost.ts`: pricing table (3 models, input/output/cache rates, dated
      source comment), token→EUR, pre-stage estimators; vitest
- [x] `prompts/system.ts`: strategist system prompt (SPEC §4.3 hard rules),
      >1024 tokens for cacheability, `cache_control` on system + intake
- [x] `prompts/schemas.ts`: JSON schemas for stage 1 (strategy) and stage 2
      (calendar) — additionalProperties:false, required fields per SPEC
- [x] `prompts/stage1-strategy.ts` / `stage2-calendar.ts` /
      `stage3-copy.ts`: prompt builders taking intake + prior stage JSON;
      stage 3 batched by week
- [x] Client-side schema validation + single retry with error feedback
- [x] Wire Generation.tsx to real calls: streaming render, cost meter on
      real usage numbers, per-stage confirm with estimate
- [x] Constraint checks in UI: budget split sums, weekly minutes vs user
      hours — flag violations, offer regenerate
- [x] Regenerate per stage with downstream-stale marking
- [ ] Prompt quality pass: run ≥3 real personas (use own key), iterate
      ...
      (pending: run 3 personas with real key)
- [x] Playwright: happy path with intercepted/mocked api.anthropic.com;
